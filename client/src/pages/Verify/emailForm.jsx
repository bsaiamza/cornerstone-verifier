...
const NewProofRequestEmailForm = () => {
  const [submitting, setSubmitting] = useState(false)

  const handleEmailSubmit = async values => {
    setSubmitting(true)
    let apiURL = '/api/v1/cornerstone/verifier/email-proof'

    if (process.env.API_BASE_URL) {
      apiURL = process.env.API_BASE_URL + '/cornerstone/verifier/email-proof'
    }

    if (!process.env.NODE_ENV || process.env.NODE_ENV === 'development') {
      await toast.promise(
        axios
          .post(process.env.REACT_APP_API_URL + 'email-proof', values)
          .then(response => {
            toast.success('Sent proof request via email!')
          })
          .catch(error => {
...
export default NewProofRequestEmailForm
