...
const vaccineURL = IAMZA_VERIFIER_URL + '/verify-vaccine'
...
const Verify = () => {
	...
	const [vaccineCredential, setVaccineCredential] = useState<string | undefined>('')
	...
	const handleVaccine = async () => {
		await toast.promise(
			axios
				.get(vaccineURL)
				.then((response) => {
					setVaccineCredential(response.data.proofRequest)
					toast.success('Generated verification request!')
				})
				.catch((error) => console.log(error)),
			{
		...
		<Grid container spacing={1}>
			...
			<Grid item container xs={12} md={4} direction='column' alignItems='center' justifyContent='center'>
				...
				<Card
					...
					elevation={3}>
					<CardActionArea sx={{ textAlign: 'center', p: 1 }} onClick={handleVaccine}>
						<img src={vaccine} height='100' width='100' alt='vaccine' />
						<CardContent>
							<Typography gutterBottom variant='subtitle2' component='div' color='text.secondary'>
								Vaccine Credential
							</Typography>
					...
					{vaccineCredential ? (
						<>
							...
							<div
								...
								<QRCode value={vaccineCredential} />
							</div>
						</>
					) : (
						''
